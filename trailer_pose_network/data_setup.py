...
class TractorTrailerData(Dataset):
    def __init__(self,
            ...
            Arguments:
                    ...
                    transform_img (callable, optional): 
                            Transform to be applied to image data. Default is None.
                    transform_data (callable, optional): 
                            Transform to be applied to other data (e.g., IMU or CAN inputs). Default is None.
            ...
            self.transorm_data = transform_data
            ...
            # apply data transform to final modified dataframe and update the dataframe.
            # NOTE: __getitem__ will automatically get the normalized data, no transforms are needed in __getitem__
            if transform_data:
                # NOTE: Right now supports basic z-score normalization and manually normalizes all possible CAN and IMU inputs
                self.df["steer_ang"] = pd.Series((self.df["steer_ang"] - np.mean(self.df["steer_ang"])) / np.std(self.df["steer_ang"]))
                self.df["vx"] = pd.Series((self.df["vx"] - np.mean(self.df["vx"])) / np.std(self.df["vx"]))
                self.df["imu_accel_x"] = pd.Series((self.df["imu_accel_x"] - np.mean(self.df["imu_accel_x"])) / np.std(self.df["imu_accel_x"]))
                self.df["imu_accel_y"] = pd.Series((self.df["imu_accel_y"] - np.mean(self.df["imu_accel_y"])) / np.std(self.df["imu_accel_y"]))
                self.df["imu_accel_z"] = pd.Series((self.df["imu_accel_z"] - np.mean(self.df["imu_accel_z"])) / np.std(self.df["imu_accel_z"]))
                self.df["imu_gyro_x"] = pd.Series((self.df["imu_gyro_x"] - np.mean(self.df["imu_gyro_x"])) / np.std(self.df["imu_gyro_x"]))
                self.df["imu_gyro_y"] = pd.Series((self.df["imu_gyro_y"] - np.mean(self.df["imu_gyro_y"])) / np.std(self.df["imu_gyro_y"]))
                self.df["imu_gyro_z"] = pd.Series((self.df["imu_gyro_z"] - np.mean(self.df["imu_gyro_z"])) / np.std(self.df["imu_gyro_z"]))

            plt.figure
            plt.hist(self.df["vy"], bins=100, edgecolor='k')
            plt.show()
            # stop=1
    ...
    def __getitem__(self, idx):
            ...
            if self.sequential is not None:
                    ...
                    if self.inputs["imu"]:
                            ...
                            #             torch.tensor(seq_block["imu_gyro_x"].to_list()),torch.tensor(seq_block["imu_gyro_y"].to_list()),torch.tensor(seq_block["imu_gyro_z"].to_list())]).permute(1,0).unsqueeze(dim=1).unsqueeze(dim=2)
                            ...
                            # input_imu = torch.stack([torch.tensor(seq_block["imu_accel_y"].to_list()),
                            #             torch.tensor(seq_block["imu_gyro_z"].to_list())]).permute(1,0).unsqueeze(dim=1).unsqueeze(dim=2)
                                                        
                            # input_imu = torch.tensor(seq_block["imu_gyro_z"].to_list()).unsqueeze(dim=1)
                    ...
                    if self.transform_img:
                            input_image = self.transform_img(input_image)
    ...
    def customOutputs(self, seq_block):
            ...
            yaw = seq_block["yaw"].iloc[0]
            hitch = seq_block["hitch"].iloc[0]
            hitch_rate = seq_block["hitch_rate"][0]
            pose1 = (seq_block["X"].iloc[1], seq_block["Y"].iloc[1], seq_block["yaw"].iloc[1])
            pose2 = (seq_block["X"].iloc[0], seq_block["Y"].iloc[0], seq_block["yaw"].iloc[0])
