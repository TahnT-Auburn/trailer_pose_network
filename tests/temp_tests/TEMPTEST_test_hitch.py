...
WEIGHT_FILE = "TEMPTEST_HITCH_async_space_time_cross_attn_v1.pth"
...
HITCH_WEIGHT_PATH = os.path.join(WEIGHT_PARENT, HITCH_WEIGHT_FILE)
...
def test():
    ...
    hitch_state_dict = torch.load(HITCH_WEIGHT_PATH)
    ...
    with torch.no_grad():
        ...
        for t, (x,y) in enumerate(tqdm(test_loader)):
            ...
            hitch_est = hitch_model(latest_image)
...
if __name__ == "__main__":
    ...
    plt.plot(hitch)
    plt.plot(hitch_truth, '--')
    plt.ylabel("Hitch prediction")
    plt.legend(["Est", "Truth"])
    plt.show()
