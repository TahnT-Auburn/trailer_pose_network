...
def test():
    ...
    with torch.no_grad():
        ...
        # for t, (x,y) in enumerate(test_loader):
            ...
            x[2] = x[2].to(device=device, dtype=torch.float32) # yaw history (truth)
            ...
            yaw_est_model = yaw_est.squeeze().cpu().numpy()
            yaw_est_model = np.rad2deg(np.arctan2(yaw_est_model[0], yaw_est_model[1]))
            yaw_truth = y[0][-2:].cpu().numpy()
            yaw_truth = np.rad2deg(np.arctan2(yaw_truth[0], yaw_truth[1]))
            rotation_est_model = np.rad2deg(rot_est.squeeze().cpu().numpy())
            rotation_truth = np.rad2deg(y[0][2].cpu().numpy())
            
            # print(f'yaw truth: {yaw_truth}')
            # print(f'yaw estimate: {yaw_est_model}')
            # print(f'Rotation prediction: {rotation_est_model}')
            # print(f'True Rotation: {rotation_truth}')
            # print()
